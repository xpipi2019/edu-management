  ...
  >
    ...
    >
      ...
      <el-form-item label="入学年份" prop="enrollment_year">
        <el-select
          v-model="formData.enrollment_year"
          placeholder="请选择入学年份"
          style="width: 100%"
        >
...
import type { Student, Department, CreateStudentData, UpdateStudentData } from '@/types/database'
...
// 表单数据
const formData = reactive<CreateStudentData & { confirmPassword?: string }>({
  user_data: {
    username: '',
    password: '',
    email: '',
...
const formRules: FormRules = {
  ...
  student_no: [
    ...
    { min: 8, max: 20, message: '学号长度为 8 到 20 个字符', trigger: 'blur' }
  ],
  dept_id: [
...
// 填充表单数据
const fillFormData = (student: Student) => {
  Object.assign(formData, {
    user_data: {
      ...
      status: student.user?.status || UserStatus.ACTIVE
    },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value && props.student) {
      ...
      await studentApi.update(props.student.student_id, updateData)
